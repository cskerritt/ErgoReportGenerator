# Define a list of variables to be included in the strategies section of all reports
import dictionary as dictionary
import docx as doc

common_strategies = [
    'Take micro-breaks from repetitious activities or static postures every 30 minutes for one or two minutes before '
    'resuming that activity or posture. Find opportunities to get out of your chair and move around.',
    'Place the telephone on your non-dominant hand side. Your dominant hand will be free for writing, and cradling '
    'the telephone between your ear and shoulder while writing will not be necessary.',
    'Use a telephone headset or speaker when performing tasks simultaneously with the telephone. This practice will '
    'prevent awkward neck and shoulder postures associated with cradling the telephone between your ear and shoulder.',
]
...
# Patch pyth/plugins/rtf15/reader.py for Python 3 compatibility
if sys.version_info.major == 3:
    from pyth.plugins.rtf15.reader import Rtf15Reader

    def _get_hyperlink_destination(self, field):
        ...
        return ""
    Rtf15Reader._get_hyperlink_destination = _get_hyperlink_destination
else:
    from pyth.plugins.rtf15.reader import Rtf15Reader
...
# Create a dictionary to store the responses and strategies
responses = {}
strategies = {}
...
for key, item in checklist.items():
    ...
    if response.lower() == 'no':
        strategies[key] = checklist[key]['strategies']

# Iterate over the responses and update the report as needed
strengths_list = []
opportunities_list = []
for key, response in responses.items():
    if response == 'yes':
        strengths_list.append(checklist[key]["yes_option"])
    else:
        opportunities_list.append(checklist[key]["no_option"])

# Get the file path from the user
root = tk.Tk()
root.withdraw()
file_path = filedialog.askdirectory(title='Select folder to save report')

# Get the document name from the user
document_name = 'report.rtf'
if os.path.isfile(document_name):
    response = input(f'The file {document_name} already exists. Do you want to overwrite it? Enter "Yes" or "No": ')
    if response.lower() == 'no':
        document_name = filedialog.asksaveasfilename(defaultextension='.rtf', filetypes=[('Rich Text Format', '*.rtf')],
                                                     title='Save Workstation Ergonomics Self-Assessment Report as',
                                                     initialdir=file_path)

# Create a Word document template
template = DocxTemplate(os.path.join(file_path, 'template.docx'))

# Define a context for the template with the report data
context = {
    'name': name,
    'position': position,
    'shift': shift,
    'image_tag': InlineImage(template, os.path.join(file_path, 'image_tag.png')),
    'strengths': [{'text': s} for s in strengths_list],
    'opportunities': [{'text': o} for o in opportunities_list],
    'strategies': [{'text': s} for s in strategies.values()]
}

# Render the template with the context to generate the report
template.render(context)

# Save the modified document as RTF
doc = Rtf15Reader.read(open(os.path.join(file_path, 'template.docx'), 'rb'))
with open(document_name, 'w') as f:
    f.write(PlaintextWriter.write(doc).getvalue())

# Open the document
subprocess.call(('open', document_name))
